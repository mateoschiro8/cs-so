...
static ssize_t azar_read(struct file *filp, char __user *data, size_t size, loff_t *offset) {
    ...
    printk(KERN_ALERT "UPPER BOUND: %d\n", upper_bound);
    ...
    random = abs(random) % upper_bound;

    char* res = kmalloc(size, GFP_KERNEL);

    size_t resSize = (size_t) snprintf(res, size, "%d\n", random);

    printk(KERN_ALERT "Mandando nro: %s", res);

    int finalSize = min(resSize, size);
    copy_to_user(data, res, finalSize);
...
static ssize_t azar_write(struct file *filp, const char __user *data, size_t size, loff_t *offset) {
    ...
    char* texto = kmalloc(size + 1, GFP_KERNEL);
    copy_from_user(texto, data, size);
    texto[size] = '\0';

    int num;
    if(kstrtoint(texto, 10, &num) == 0) {
        upper_bound = num;
        printk(KERN_ALERT "Nuevo numero es: %d\n", num);
        kfree(texto);
	return sizeof(num);
    }
